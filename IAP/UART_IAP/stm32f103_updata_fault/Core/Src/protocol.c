...
//	  STMFLASH_Write(g_download_addr,&pudata[9],m_data_len);	
	  iap_write_appbin(g_download_addr,&pudata[9],m_data_len);

    /*printf("g_download_addr=%08x\n",g_download_addr);
      printf("m_firmware_len=%08x\n",m_firmware_len);
	    printf("m_data_offset=%08x\n",m_data_offset);
	    printf("m_data_len=%d\n",m_data_len);*/
    ...
    uart3_send_chars(temp_send_data, temp_send_data[2]);
     ...
     if(m_resp_flag==0xA1)
     {
        ...
        for(i=0;i<loop_len;i++)
        {
          for(j=0;j<255;j++)
          {   
              ar_updata_buff[j]=STMFLASH_ReadWord(UPDATA_BACKA_ADDRESS+255*i+j);            
          }
        ...
        {
           ar_updata_buff[j]=STMFLASH_ReadWord(UPDATA_BACKA_ADDRESS+255*loop_len+j);
        }
...
//		       STMFLASH_Write(UPDATA_BACKC_ADDRESS,ar_upsign_buff,12);//???????crc32?bin?????
           iap_write_appbin(UPDATA_BACKC_ADDRESS,ar_upsign_buff,12); 
